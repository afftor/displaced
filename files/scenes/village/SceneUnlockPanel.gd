...
func testmode():
	...
	input_handler.scene_node = tnode
	tnode.hide()
	add_child(tnode)
...
func rebuild_scene_list():
	...
	for event in events.events:
		...
		if selected_char != 'all' and !eventdata.cost.has(selected_char): continue
		if state.gallery_event_unlocks.has(event):
			...
			continue
		if state.OldEvents.has(event) and eventdata.has('auto_unlocks') and eventdata.auto_unlocks:
			state.gallery_event_unlocks.push_back(event)
			var panel = input_handler.DuplicateContainerTemplate(scenelist, 'Button')
			panel.get_node('Image').texture = resources.get_res(eventdata.icon)
			panel.get_node('Label').text = "{name}\n{description}".format(eventdata)
			panel.connect('pressed', self, 'show_event', [event])
		...
		if state.OldEvents.has(event): #shown and not unlocked == not unknown
			...
			continue
		if eventdata.has("unlock_before_seen") and eventdata.unlock_before_seen and state.valuecheck(eventdata.unlock_conditions): #not shown but unlockable
			var panel = input_handler.DuplicateContainerTemplate(scenelist, 'Button2')
			var can_unlock = true
			panel.get_node('Image').texture = resources.get_res(eventdata.icon)
			panel.get_node('Label').text = "{name}\n{description}".format(eventdata)
...
func unlock_show_event(ev):
	var eventdata = events.events[ev]
	for ch in eventdata.cost:
		var hero = state.heroes[ch]
		hero.friend_points -= eventdata.cost[ch]
	state.gallery_event_unlocks.push_back(ev)
	show_event(ev)
	rebuild_scene_list()
