...
#	input_handler.ClearContainer($HiddenContainer/GridContainer)
	...
	for i in state.materials:
		...
		newbutton.set_meta('type', material.itemtype)
...
func rebuildinventory():
	for btn in itemarray:
		if category == 'all' || btn.get_meta('type') == category:
			btn.show()
		else:
			btn.hide()
#	to delete
...
#all beneath probably useless, as all it's functions already implemented elsewhere
