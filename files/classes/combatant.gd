...
func apply_temp_effect(eff_id):
	...
	if check_status_resist(eff):
		if input_handler.combat_node != null:
			input_handler.combat_node.combatlogadd(tr("IS_RESISTS") % [get_stat('name'), tr(eff.template.name)])
			play_sfx('sfx_dispel')
		return
	if input_handler.combat_node != null:
		input_handler.combat_node.combatlogadd(tr("IS_AFFLICTED") % [get_stat('name'), tr(eff.template.name)])
