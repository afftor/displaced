...
var buffs = {} #for display purpose ONLY
...
func apply_atomic(tmp):
	match tmp.type:
		...
		'block_effect', 'delete_effect':
			remove_effect(tmp.effect);
			pass
		'damage':
			deal_damage(tmp.value, tmp.source);
	pass

func remove_atomic(code):
	var tmp = Effectdata.atomic[code];
	match tmp.type:
		'stat':
			set(tmp.stat, get(tmp.stat) - tmp.value);
			pass
...
func on_skill_check(skill, target, is_crit = false): #skill has to be in constant form without metascripting. this part has to be done in conbat.gd in execute_skill
	#var tt = Skillsdata.skilllist[skill].copy();
	for e in triggered_effects:
		var tmp = Effectdata.effect_table[e];
		if tmp.trigger != 'skill_hit': continue;
		#check conditions
		var res = true;
		for cond in tmp.conditions:
			match cond.target:
				'skill':
					...
					pass
				'self':
					res = res and input_handler.requirementcombatantcheck(cond.value, self);
					pass
				'target':
		...
		for ee in tmp.effects:
			var eee = Effectdata.atomic[ee].copy();
			...
			if eee.type == 'skill':
				...
				pass
			var rec = self;
			if eee.receiver == 'target': rec = target;
			rec.apply_atomic(eee);
		pass
	pass

func on_def_check(caster, skill, is_crit = false):
	for e in triggered_effects:
		var tmp = Effectdata.effect_table[e];
		if tmp.trigger != 'def_hit': continue;
		#check conditions
		var res = true;
		for cond in tmp.conditions:
			match cond.target:
		...
		for ee in tmp.effects:
			var eee = Effectdata.atomic[ee];
			var rec = self;
			if eee.receiver == 'caster': rec = caster;
			rec.apply_atomic(eee);
		pass
	pass
